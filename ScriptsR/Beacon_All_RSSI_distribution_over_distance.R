...
rssi_all_beacons_over_distance <- function(path, beacon_name){
  ...
  boxplot(rssi_data,main=paste("RSSI Distribution over distance - Beacon",beacon_name),xlab="Distance [m]", ylim=c(-105,-60), ylab="RSSI [dBm]",las=1, xaxt="n")
...
plot(x, promedio_rssi_1, type="n", main = "Mean RSSI evolution over distance",  xlab="Distance [meters]", ylab="RSSI [dBm]",las=1, xlim=c(0,18), ylim=c(-95,-65), xaxt="n")
axis(1, at=c(0:17), labels=seq(0,8.5, 0.5))
...
legend(8,-61, c("D0:F8:F2:95:3A:5C","D6:40:86:6D:BD:DF","D9:A3:28:6A:A7:F1","F5:7A:A5:E3:EF:18","FF:A6:7C:AF:B0:CB"), lwd=1, col=c('red', 'blue', 'green4','cornflowerblue','orange'), bty='n', xjust=0, x.intersp=0.1, y.intersp=0.2, seg.len=0.10, cex=1)