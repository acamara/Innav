...
plot(0, 0, type="n", main="Test environment - Building 1 (with estimate distances and NLSQ method)", xlab="x (10.67 m)", ylab="y (10.37 m)", xlim=c(0,ceiling(roomWidth)), ylim=c(0,ceiling(roomHeight)), xaxt="n", yaxt="n")
...
for(i in 1:length(B1_accuracy_data)){
  sol = calculate_Position_NLSQ(b1x, b1y, b4x, b4y, b5x, b5y, B1_accuracy_data[[i]], B4_accuracy_data[[i]], B5_accuracy_data[[i]])
  x_positions[i] = sol[1];
  y_positions[i] = sol[2];
}
