...
plot(0, 0, type="n", main="Test environment - Room 1 (with estimate distances and NLSQ method)", xlab="x (6.00 m)", ylab="y (4.40 m)", xlim=c(0,ceiling(roomWidth)), ylim=c(0,ceiling(roomHeight)), xaxt="n", yaxt="n")
...
for(i in 1:length(B1_accuracy_data)){
  sol = calculate_Position_NLSQ(b1x, b1y, b2x, b2y, b3x, b3y, B1_accuracy_data[[i]], B2_accuracy_data[[i]], B3_accuracy_data[[i]])
  x_positions[i] = sol[1];
  y_positions[i] = sol[2];
}
